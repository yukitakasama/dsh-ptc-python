...
/**
 * The backend's validated config. Every cap is deployment-varying and therefore
 * a config field rather than a constant; the internal interface width caps live
 * in the code because they bound only the host's own re-serialization.
 */
export const Config = z.object({
  pythonPath: z.string().default(''),
  /**
   * Interpreter flags placed before the bootstrap path. `-I` is the default
   * because it is the isolation this backend documents: no user site-packages,
   * no `PYTHONPATH`/`PYTHONHOME`. An empty list runs the interpreter fully
   * unisolated, which a deployment can choose deliberately.
   */
  isoFlags: z.array(z.string()).default(['-I']),
  maxWallMs: z.number().default(300_000),
  maxComputeMs: z.number().default(0),
  maxOutputBytes: z.number().default(67_108_864),
  maxFrameBytes: z.number().default(8_388_608),
  cpuSeconds: z.number().default(60),
  addressSpaceBytes: z.number().default(2_147_483_648),
  maxLogBytes: z.number().default(1_048_576),
  maxValueBytes: z.number().default(4_194_304),
  bootTimeoutMs: z.number().default(30_000),
  protocol: z.union(['stdin-stdout']).default('stdin-stdout'),
})
